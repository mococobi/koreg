(function () {
    ...
    mstrmojo.plugins.OffLineMap2.OffLineMap2 = mstrmojo.declare(mstrmojo.CustomVisBase, null, {
        ...
        plot: function () {
            ...
            function validateLatLngData(latLngData) {
                if (!latLngData) { return false; }
                for (var i = 0; i < latLngData.length; i++)  {
                    if (isNaN(latLngData[i].lat) || isNaN(latLngData[i].lng)) { return false; }
                };
                return true;
            }
            ...
            function addLabelText() {
                ...
                */
                if (!featureLabel || labelAttrIndex == -1) { return; }
            ...
            function renderGeoJson() {
                ...
                function makeGeoLayer(error, loadJsonFeature) {
                    ...
                    for (var i = 0; i < loadJsonFeature.features.length; i++) {
                        if (codeMap.get(loadJsonFeature.features[i].properties.CD)) {
                    ...
                    geoJson = L.geoJson(
                                    geoJsonFeature, {
                                        style: function(feature) { 
                                            return getGeoStyle( getMetricColor(codeMap.get(feature.properties.CD)) );
                                        },
                                        className: "geoRegion", 
                    ...
                    if (_p_fitRegion === "Data" && geoJson.getBounds().isValid()) { // fit to current features ;
                ...
                var codeMap = undefined;
                if (codeAttrIndex != -1) { 
                    codeMap = d3.map(data, function (d) {return d.code}); 
                } else {
                    return;
                }
            ...
            function renderMarker() {
				...
				if (_p_fitRegion === "Data" && validateLatLngData(data)) { mymap.fitBounds( L.latLngBounds( getLatLngData(data) )); }
            ...
            var _p_mapOpacity = this.getProperty("mapOpacity");
            ...
            // 2023-04-06,PDF 인쇄 시 선택영역 어긋난 위치조정
            var mymap = L.map( divId, {renderer:L.svg({padding:0.0, tolerance:0})}).setView([36, 128], 8 );
            ...
            if (_p_mapUrl) {
                L.tileLayer(_p_mapUrl + "/Tiles/{z}/{x}/{y}.png", { // L.tileLayer("/MicroStrategy/Map/Tiles/{z}/{x}/{y}.png", {
                    maxZoom: 15,
                    minZoom: 5,
                    opacity: _p_mapOpacity,
                    errorTileUrl: _p_mapUrl + "Map/Tiles/errorTile.png",
