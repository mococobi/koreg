    	...
    	<table id="board_table" class="table table-sm table-bordered" style="width: 100%; border: 1px solid; border-collapse: collapse;">
			...
			<tbody>
				<tr>
					...
					<td colspan="7">
						<span id="post_title">${postData['POST_TITLE']}</span>
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<span id="post_create_user_nm">${postData['CRT_USR_NM']}</span>
					</td>
					<td>
						<span>부서</span>
					</td>
					<td>
						<span id="post_create_user_dept_name">${postData['CRT_USR_DEPT_NM']}</span>
					</td>
					<td>
						<span>작성일자</span>
					</td>
					<td>
						<span id="post_create_date"><fmt:formatDate value="${postData['CRT_DT_TM']}" pattern="yyyy-MM-dd"/></span>
					</td>
					<td>
						<span>조회</span>
					</td>
					<td>
						<span id="post_count">${postData['POST_VIEW_COUNT']}</span>
					</td>
				</tr>
				<c:if test="${boardData['POST_TYPE_YN'] eq 'Y'}">
					<tr>
						<td>
							<span>분류</span>
						</td>
						<td colspan="7">
							<span id="post_type_nm">${postData['POST_TYPE']}</span>
						</td>
					</tr>
				</c:if>
				<c:if test="${boardData['POST_POPUP_YN'] eq 'Y'}">
					<tr id="post_popup_yn_div">
						...
						<td class="text-center">
							<c:choose>
								<c:when test="${postData['POPUP_YN'] eq 'Y'}">
									<input type="checkbox" id="popup_yn" checked="checked" disabled>
								</c:when>
								<c:otherwise>
						...
						<td colspan="5">
							<span id="post_popup_dt">
								<fmt:formatDate value="${postData['POPUP_START_DT_TM']}" pattern="yyyy-MM-dd"/> 
								~
								<fmt:formatDate value="${postData['POPUP_END_DT_TM']}" pattern="yyyy-MM-dd"/> 
							</span>
						</td>
					</tr>
				</c:if>
				<tr>
					<c:if test="${boardData['POST_FIX_YN'] eq 'Y'}">
						...
						<td class="text-center">
							<c:choose>
								<c:when test="${postData['POPUP_YN'] eq 'Y'}">
									<input type="checkbox" id="fix_yn" checked="checked" disabled>
								</c:when>
								<c:otherwise>
				...
				<tr>
					...
					<td colspan="7">
						<div id="post_content" style="min-height: 300px;">
					...
					<tr id="post_file_yn_div">
						...
						<td colspan="7">
							<div id="post_file" class="list-group">
								<c:forEach items="${file}" var="attachFile" varStatus="status">
									<a class="list-group-item list-group-item-action list-group-item-secondary" style="cursor: pointer;" onclick="${attachFile}">
										${attachFile['ORG_FILE_NM']} + '.' + ${attachFile['FILE_EXT']} + '\t' + formatFileSize(${attachFile['FILE_SIZE']})
									</a>
								</c:forEach>
							</div>
						</td>
...
// 			callBoardPostDetail();
	...
	function displayBoardPostContents(postData, postFile, postLocation) {
		...
		if($('#post_type_nm').length == 1) {
			$('#post_type_nm').text(postData['POST_TYPE']);
		}
		
		if($('#popup_yn').length == 1) {
